...
evo2_image = (
    ...
    .run_commands("pip install 'transformer-engine[pytorch]==1.13' --no-build-isolation")
...
@app.function(gpu="H100", volumes={mount_path: volume}, timeout=1800)
...
def brca1_example():
    ...
    print("Running BRCA1 example with EVO2...")

    # run inference
    result = run_brca1_analysis.remote()

    if "plot" in result:
        # The plot is already base64 encoded from run_brca1_analysis
        # No need to encode it again, just decode it for use
        with open("brca1_analysis_plot.png", "wb") as f:
            f.write(base64.b64decode(result["plot"]))

        # show plot from saved file
        img = mpimg.imread("brca1_analysis_plot.png")
...
def get_genome_sequence(position, genome:str, chromosome:str, window_size=8192):
    ...
    print(f"Fetching {window_size}bp window arounf position {position} from UCSC API...")
    print(f"Coordinates: {chromosome}: Start: {start} - End: {end} ({genome})")
    ...
    if response.status_code != 200:
        raise Exception(
            f"Failed to fetch sequence from UCSC API: {response.status_code} - {response.text}"
            )
    ...
    if len(sequence) != expected_length:
        print(
            f"Unexpected sequence length: {len(sequence)} (expected {expected_length})"
        )

    print(f"Fetched sequence length: {len(sequence)} bases")
    return sequence, start

def analyze_variant(relative_position, reference, alternative, window_seq, model):
    var_seq = window_seq[:relative_position] + alternative + window_seq[relative_position+1:]
...
class Evo2Model:
    ...
    def analyze_single_variant(self, variant_position: int, alternative: str, genome: str, chromosome: str):
        ...
        print(f"Fetched genome seq window, first 100 bases: {window_seq[:100]}...")

        relative_position = variant_position - seq_start
        print(f"Relative position within window: {relative_position}")

        if relative_position < 0 or relative_position >= len(window_seq):
            raise ValueError(
                f"Variant position {variant_position} is out of bounds for the fetched sequence window, start: {seq_start}, end: {seq_start + len(window_seq)}"
            )
        
        reference = window_seq[relative_position]
        print(f"Reference base at position {variant_position}: {reference}")
